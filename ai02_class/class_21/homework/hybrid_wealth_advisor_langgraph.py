...
llm = ChatTongyi(model_name="qwen-max", dashscope_api_key=DASHSCOPE_API_KEY)
...
# ============================================================
# 财富管理知识库 + 两种检索工具（BM25 / 向量语义）
# ============================================================

# 简化的财富管理知识库（理财概念、产品、政策）
WEALTH_KB = [
    {"title": "ETF基金", "content": "ETF（交易型开放式指数基金）是在交易所上市交易、份额可变的开放式基金。它通常被动跟踪某一指数，具有费率低、透明度高、交易灵活（可像股票一样盘中买卖）的特点，适合作为长期配置的底层工具。"},
    {"title": "基金定投", "content": "基金定投是指在固定时间（如每月）以固定金额投资指定基金。它能平摊买入成本、分散择时风险，适合工薪族强制储蓄和长期理财，是穿越牛熊的常见纪律化策略。"},
    {"title": "债券投资", "content": "债券是政府或企业发行的、约定到期还本付息的固定收益类工具。相比股票波动更小、现金流更稳定，常用于降低组合整体风险、提供稳定票息，是稳健型和保守型投资者的配置重点。"},
    {"title": "可转换债券", "content": "可转债是一种可按约定条件转换为发行公司股票的债券，兼具债性（保底票息）和股性（上涨弹性），具有“下有保底、上不封顶”的特点，适合风险偏好适中、希望进可攻退可守的投资者。"},
    {"title": "资产配置与再平衡", "content": "资产配置是把资金按比例分散到股票、债券、现金、另类等不同类别以分散风险。再平衡是定期把偏离目标比例的仓位调回，从而做到高抛低吸、控制风险敞口，是长期收益的关键来源。"},
    {"title": "风险承受能力评估", "content": "风险承受能力反映投资者承受亏损的财务与心理能力，通常分为保守型、稳健型、平衡型、成长型、进取型。它由年龄、收入稳定性、投资期限、财务目标等决定，是制定资产配置方案的前提。"},
    {"title": "合格投资者标准", "content": "私募基金合格投资者需具备相应风险识别和承担能力，投资单只私募金额不低于100万元，且净资产不低于1000万元（单位）或金融资产不低于300万元、近三年年均收入不低于50万元（个人）。"},
    {"title": "个人养老金与税延", "content": "个人养老金账户每年可享受税前扣除额度（目前为每年12000元），投资收益暂不征税，领取时按较低税率计税，可投资符合规定的养老储蓄、理财、基金、保险产品，是长期退休规划的税优工具。"},
    {"title": "黄金与避险配置", "content": "黄金与主要风险资产相关性较低，在通胀上行或市场剧烈波动时往往具有避险价值。适度配置黄金（如通过黄金ETF）有助于平滑组合波动，但其本身不产生利息或股息。"},
    {"title": "美元与外汇资产", "content": "配置美元或海外资产可以分散单一市场和单一货币风险，对冲本币贬值。常见方式包括QDII基金、海外ETF等，但需关注汇率波动、跨境政策及额度限制。"},
]

# --- BM25 索引（启动时构建，纯本地、无需联网）---
_kb_corpus = [doc["title"] + doc["content"] for doc in WEALTH_KB]
_kb_tokenized = [list(jieba.cut(text)) for text in _kb_corpus]
_bm25 = BM25Okapi(_kb_tokenized)

# --- 向量索引（懒加载：首次语义检索时才请求 embedding）---
_kb_embeddings: Optional[np.ndarray] = None


def _embed_texts(texts: List[str]) -> np.ndarray:
    """调用 DashScope text-embedding-v2 批量获取文本向量"""
    resp = dashscope.TextEmbedding.call(model="text-embedding-v2", input=texts)
...
def _ensure_kb_embeddings() -> np.ndarray:
    """懒加载知识库向量，只在首次调用时计算并缓存"""
    global _kb_embeddings
    if _kb_embeddings is None:
        print("[向量检索] 首次调用，正在为知识库构建向量索引...")
        _kb_embeddings = _embed_texts(_kb_corpus)
    return _kb_embeddings


@tool
def search_knowledge_bm25(query: str) -> str:
    """使用 BM25 关键词检索财富管理知识库。
    适合包含明确术语/产品名的提问（如“ETF”“可转债”“个人养老金”），基于字面词频匹配，速度快、无需联网。
    ...
    hits = [i for i in top_idx if scores[i] > 0]
    if not hits:
        return "BM25检索：知识库中未找到相关内容。"

    parts = [f"【BM25检索结果】"]
    for i in hits:
        parts.append(f"- {WEALTH_KB[i]['title']}（相关度{scores[i]:.2f}）：{WEALTH_KB[i]['content']}")
    result = "\n".join(parts)
    print(f"[工具调用] search_knowledge_bm25('{query}') -> 命中{len(hits)}条")
    return result


@tool
def search_knowledge_semantic(query: str) -> str:
    """使用向量语义检索财富管理知识库。
    懂近义和语义，适合口语化、概念性的提问（如“怎么强制自己存钱投资”能匹配到“基金定投”），即使没有相同字眼也能命中。

    Args:
        query: 用户的查询问题（自然语言）
    """
    kb_vecs = _ensure_kb_embeddings()
    q_vec = _embed_texts([query])[0]

    # 余弦相似度
    sims = kb_vecs @ q_vec / (np.linalg.norm(kb_vecs, axis=1) * np.linalg.norm(q_vec) + 1e-8)
    top_idx = np.argsort(sims)[::-1][:2]

    parts = ["【语义检索结果】"]
    for i in top_idx:
        parts.append(f"- {WEALTH_KB[i]['title']}（相似度{sims[i]:.2f}）：{WEALTH_KB[i]['content']}")
    result = "\n".join(parts)
    print(f"[工具调用] search_knowledge_semantic('{query}') -> top相似度{sims[top_idx[0]]:.2f}")
    return result
...
你可以使用以下工具来获取实时数据或查询知识库:
- query_shanghai_index: 查询上证指数实时行情
- query_portfolio_allocation: 查询投资组合中特定资产的配置比例
- query_market_news: 查询最新市场新闻
- search_knowledge_bm25: 用 BM25 关键词检索财富管理知识库（用户问题含明确术语/产品名时优先用，如“ETF”“可转债”）
- search_knowledge_semantic: 用向量语义检索财富管理知识库（用户口语化、概念性提问时优先用，能匹配近义表达）

请根据用户问题判断是否需要调用工具，如果需要请调用相应工具获取数据后再回答。对于理财概念/产品类的解释性问题，应优先使用知识库检索工具。"""
...
if __name__ == "__main__":
    ...
    SAMPLE_QUERIES = [
        ...
        "今天上证指数的表现如何？",
        "我的投资组合中股票占比是多少？",
        "请解释一下什么是ETF？",  # 含术语，模型倾向 BM25 检索
        "我是工薪族，怎么能强制自己每月存钱做投资？",  # 口语化，需语义检索匹配“基金定投”
